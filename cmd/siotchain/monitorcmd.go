...
func monitor(ctx *cli.Context) error {
	...
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to siotchain node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by siotchain (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached siotchain node and retrieves the entire set

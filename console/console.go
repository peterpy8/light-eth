...
func (c *Console) init(preload []string) error {
	...
	flatten := "var siot = web3.siot; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = siot.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines siot.getBalance(siot.coinb<tab><tab>
	start := 0
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + siot.coinbase);
		console.log("at block: " + siot.blockNumber + " (" + new Date(1000 * siot.getBlock(siot.blockNumber).timestamp) + ")");
